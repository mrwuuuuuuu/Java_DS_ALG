...
public class LinkedListALG {
	/**
	 * 8.1.7 如何检测一个链表是否有环 
	 * 算法：一个快指针和一个慢指针，快指针每次走两步，慢指针每次走一半，两个指针同时向前移动，
	 ...
	 * 算法：
	 * 1、先分别遍历两个链表，记录两个链表的尾结点，获得两个链表的长度len1，len2。
	 * 2、如果尾结点不等，则两个链表不相交，返回false
	 * 3、比较len1和len2的大小
	 * 4、根据len1和len2的大小来处理，取d=|len1-len2|
	 * 5、遍历较长的链表，遍历到第d个结点的时候，返回该结点
	 * @param h1
	 * @param h2
	 * @return
	 */
